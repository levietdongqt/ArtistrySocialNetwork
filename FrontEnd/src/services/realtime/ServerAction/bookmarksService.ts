...
export async function deleteBokMarksByUserIdAndPostId(postId:string,userId: string): Promise<any> {
    try {
        // console.log(headers)
        const res = await fetch(`${process.env.NEXT_PUBLIC_REALTIME_SERVICE_URL}/bookmark/deleteBookmarksById/${postId}/${userId}`,
            {
                method: 'DELETE',
                headers: await getServerSideHeaders(true, ConTentType.JSON),
                cache: "no-cache"
