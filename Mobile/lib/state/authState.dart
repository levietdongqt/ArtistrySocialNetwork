...
class AuthState extends AppState {
  ...
  void logoutCallback() async {
    cprint(await getIt<SharedPreferenceHelper>().getUserProfile(),
        errorIn: "LOGOUT 1");
    ...
    myUser = null;
    user = null;
    // _profileQuery!.onValue.drain(); "Dong Sửa"
    _profileQuery = null;
    cprint("HELLO1",label: "SINGOUT");
    if (isSignInWithGoogle) {
      cprint("HELLO2",label: "SINGOUT");

      _googleSignIn.signOut();
      Utility.logEvent('google_logout', parameter: {});
      isSignInWithGoogle = false;
    }
    cprint("HELLO3",label: "SINGOUT");

    _firebaseAuth.signOut();
    cprint("HELLO",label: "SINGOUT");
  ...
  Future<String?> signIn(String phoneNumber, String password,
      {required BuildContext context}) async {
    print("FROM AUTH STATE");
    // GetIt locator = GetIt.instance;
    // MyUser userShare = locator<MyUser>();
    try {
      isBusy = true;
      ...
      if (response.status == 200) {
        ...
        myUser = MyUser.fromJson(response.data['user']);
        await getIt<SharedPreferenceHelper>().saveUserProfile(myUser!);
        await getIt<SharedPreferenceHelper>().saveAccessToken(accessToken);
        await getIt<SharedPreferenceHelper>().saveRefreshToken(refreshToken);
        // Utility.mapperUser(myUser!, userShare);
        print(myUser);
        return myUser!.id;
      }
      return null;
  ...
  Future<MyUser?> getCurrentUser() async {
    try {
      ...
      MyUser? user = await getIt<SharedPreferenceHelper>().getUserProfile();
  ...
  Future<MyUser?> getUserDetail(String userId) async {
    // UserModel user;
    ...
    try {
      ...
      ApiHelper.callApi(HttpMethod.GET, '/user/get/${userProfileId}', null,
        ...
        if (response.status == 200) {
          ...
          getIt<SharedPreferenceHelper>().saveUserProfile(myUser);
        }
        Utility.logEvent('get_profile1', parameter: response.data);
        isBusy = false;
      });
    } catch (error) {
      isBusy = false;
